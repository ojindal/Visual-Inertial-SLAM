...
# Initiallizing the updated pose with zeros and using the zeroth value as it is
updated_pose = np.zeros(pose_traj.shape)
updated_pose[:,:,0] = pose_traj[:,:,0]

updated_var = np.zeros(var_arr.shape)
updated_var[:,:,0] = var_arr[:,:,0]
...
for ti in range(t.shape[1]-1):
    ...
    inv_T = np.linalg.inv(updated_pose[:,:,ti])  # (4x4)
    ...
    mu_unflat = np.hstack((mu_obs.reshape(features.shape[1],3), np.ones(features.shape[1]).reshape(features.shape[1],1)))
    ...
    # Initiallizations
    Hu = np.zeros((4*Nt,6))
    zu_tilda = np.zeros((4*Nt,1))

    for px in range(features.shape[1]):
        if px in indGood:
            pp = np.matmul(TT,(mu_unflat[px,:].reshape(4,1)))
            z_t = np.matmul(M,(pp/mu_unflat[px,2]))
            # Updating z-tilda for update step
            zu_tilda[d,0] = z_t[0]
            zu_tilda[d+1,0] = z_t[1]
            zu_tilda[d+2,0] = z_t[2]
            zu_tilda[d+3,0] = z_t[3]

            xx = inv_T @ mu_unflat[px,:].reshape(4,1)
            mat1 = oTi @ spcdot(xx)

            # Jacobian - update step
            Hu[4*dh:(4*dh)+4,:6] = -1*M @ dpi_by_dq(pp/mu_unflat[px,2]) @ mat1
            dh += 1

    HuT = Hu.T
    mat2 = np.linalg.inv((Hu @ updated_var[:,:,ti] @ HuT) + IV)
    # Kalman gain
    Kul = updated_var[:,:,ti] @ HuT @ mat2

    uu = (Kul @ (z - zu_tilda)).reshape(6,)
    uu_h,a = u_hatting(uu[:3],uu[3:])

    updated_pose[:,:,ti+1] = updated_pose[:,:,ti] @ expm(uu_h)
    ss = np.eye(updated_var.shape[0])-(Kul @ Hu)
    updated_var[:,:,ti+1] = (ss @ updated_var[:,:,ti] @ np.transpose(ss)) + (Kul @ IV @ Kul.T)
